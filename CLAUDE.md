...
Logging is configured centrally via `backend/app/core/logging.py`
(`configure_logging(log_filename)`), which attaches a console handler and a
rotating file handler (5MB per file, 3 backups) to the root logger.

Each long-running process writes to its own file under `backend/logs/`:

```text
backend/logs/backend.log   ← FastAPI app (configured in app/main.py)
backend/logs/worker.log    ← Celery worker (configured in app/workers/celery.py)
```

`backend/` is bind-mounted into both containers, so log files land directly
on the host filesystem at `backend/logs/` with no extra Docker volume
configuration. Log files themselves are gitignored (`backend/logs/*.log`);
the directory is kept in git via `.gitkeep`.

Any new long-running entrypoint (e.g. a future worker type) should call
`configure_logging()` once at import/startup with its own filename, following
the same pattern.
